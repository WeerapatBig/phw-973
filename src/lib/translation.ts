...
async function gemini(strings: string[], langName: string): Promise<string[]> {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    if (!Array.isArray(arr) || arr.length !== strings.length) {
      throw new Error("Gemini returned an unexpected number of strings");
    }
...
export async function getCached(
  ...
  // 1. Check local/remote JSON map file if source is provided
  if (source) {
    ...
    if (strings.length > 0) {
      ...
      if (missing.length === 0) {
        return applyTranslationMap(source, localeMap);
      }
    }
  }

  // 2. Fall back to Postgres Supabase cache table
  ...
  if (data.source_hash !== hash) return null;
  return data.payload;
}
...
export async function localized<T>(
  ...
  source: T,
  locale: LocaleCode
): Promise<{ source: T; translated: T | null }> {
  if (locale === "en") return { source, translated: null };
  const cached = await getCached(scope, locale, contentHash(source), source);
  return { source, translated: (cached as T) ?? null };
}
